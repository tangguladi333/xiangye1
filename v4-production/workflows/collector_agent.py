...
def collector_agent(state: dict) -> dict:
    ...
    for query in build_github_queries():
        ...
        try:
            ...
            logger.warning("[CollectorAgent] GitHub API HTTP error for %r: %s", query, e)
            continue
        except urllib.error.URLError as e:
            logger.warning("[CollectorAgent] GitHub API connection error for %r: %s", query, e)
            continue
    ...
    for s in sources:
        ...
        if warnings:
            logger.warning("[Security] %s title 检出注入模式: %s", s.get("url", "?"), warnings)
